---
name: dev-verify
description: verify that an implementation did not break existing behavior by running the relevant existing checks in the repository. use when code changes are already made and codex should run the existing tests or project checks, fix regressions introduced by the change when possible, and report what was run and what still fails. trigger this skill for regression checking after implementation or after test additions.
---
