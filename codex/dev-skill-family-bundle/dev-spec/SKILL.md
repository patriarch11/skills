---
name: dev-spec
description: turn a freeform development request or pasted task into an approved implementation specification. use when a coding request is still ambiguous, underspecified, or missing acceptance criteria and codex should first produce a spec instead of writing code. trigger this skill for feature requests, bug fixes, refactors, integrations, and api changes when the user wants the agent to restate the work, define scope, surface assumptions, capture nfrs, and wait for explicit approval before planning or implementation.
---

# Dev Spec

Convert the user's freeform request and optional pasted task text into an implementation-ready specification. Do not write code. Do not produce an implementation plan. Stop and wait for explicit user approval before any later phase.

## Workflow

1. Parse the request and any pasted task text.
...
- The requested behavior conflicts with the pasted task, repo conventions, or earlier instructions.
